...
import { repoRoot, appendJsonl } from "./lib/runtime.ts";
...
async function validateExpectJson(absPath: string): Promise<CheckFailure | null> {
  try {
...
async function checkStructure(root: string): Promise<CheckFailure[]> {
  ...
  if (!(await Bun.file(judgePromptPath).exists())) {
    failures.push({ file: judgePromptPath, reason: "missing tests/evals/judge-prompt.md" });
  }
  ...
  try {
    ...
    failures.push({ file: domainsRoot, reason: "missing tests/evals/domains/" });
    return failures;
  }
  ...
  for (const domain of liveDomains) {
    ...
    for (const zigFile of zigFiles) {
      ...
      if (!(await Bun.file(expect).exists())) {
        failures.push({ file: zigFile, reason: `missing pair ${base}.expect.json` });
        continue;
      }
    ...
    for (const expect of expectGlob.scanSync({ cwd: domainDir, absolute: true })) {
  ...
  if (!(await Bun.file(thresholdsPath).exists())) {
    failures.push({ file: thresholdsPath, reason: "missing tests/evals/thresholds.json" });
  } else {
    try {
      ...
      failures.push({ file: thresholdsPath, reason: `invalid JSON: ${(err as Error).message}` });
    }
  }
  ...
  try {
    ...
    for (const _ of tGlob.scanSync({ cwd: trajectoriesDir, absolute: true })) trajCount += 1;
  } catch {
    /* handled below */
  }
  if (trajCount === 0) {
