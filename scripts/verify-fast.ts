...
import { tail, appendJsonl, spawnSync, type SpawnResult } from "./lib/runtime.ts";
...
function printFail(label: string, result: SpawnResult): void {
  ...
  const blob = [result.stdout, result.stderr].filter((s) => s.length > 0).join("\n");
...
async function main(): Promise<void> {
  ...
  if (version.length === 0) {
    console.error("verify-fast: could not resolve a zig toolchain (set $ZIG or install mise)");
  ...
  const hasZiglint = spawnSync(["command", "-v", "ziglint"]);
  if (hasZiglint.code === 0) {
