...
import { spawnSync, repoRoot, appendJsonl } from "./lib/runtime.ts";
...
function extractSurface(root: string, rootFile: string): string {
...
async function main(): Promise<void> {
  ...
  const baselinePath = process.env.PUBLIC_API_BASELINE ?? ".zig-qm/public-api.txt";
  ...
  if (!(await Bun.file(absRoot).exists())) {
    console.log(`(no public API root at ${rootFile}; skipping public surface check)`);
    await finish(0, startedAt);
  }

  const current = extractSurface(root, rootFile);
