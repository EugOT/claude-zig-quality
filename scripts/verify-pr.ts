...
import { repoRoot, tail, appendJsonl } from "./lib/runtime.ts";
...
async function runFuzzBounded(limit: string): Promise<"pass" | "timeout" | number> {
  const root = repoRoot();
...
async function main(): Promise<void> {
  ...
  for (const target of TARGETS) {
    ...
    if (build.code !== 0) {
      console.error(`verify-pr: cross-target build failed for ${target} (exit ${build.code ?? "?"})`);
  ...
  for (const mode of SAFETY_MODES) {
    ...
    if (test.code !== 0) {
      console.error(`verify-pr: ${mode} tests failed (exit ${test.code ?? "?"})`);
  ...
  if (hasBuildStep("docs")) {
    ...
    console.log("(no docs build step — add one so shipment checks can verify generated API docs)");
  }
  ...
  if (hasBuildStep("fuzz")) {
    ...
    console.log("(no 'fuzz' build step — skipping fuzz gate; add one per zig-fuzz-target skill)");
  }
